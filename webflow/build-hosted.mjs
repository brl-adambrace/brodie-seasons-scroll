...
const sections=[
  ...
  {name:'browser-tint',display:'browserTint',js:'browser-tint.js'},   // plain script, no CSS: keeps theme-color matched to the section beside the browser bar
];
...
for(const s of sections){
  ...
  if(s.js){ src=read(s.js); }
    ...
    src=`(function(){var c=${JSON.stringify(css)};var st=document.createElement('style');st.setAttribute('data-brl','${s.name}');st.textContent=c;(document.head||document.documentElement).appendChild(st);function init(){${js}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();})();`;
  }
